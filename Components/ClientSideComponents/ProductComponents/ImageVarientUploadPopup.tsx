"use client";
import { imgAllDataApi, imgAllVArientDataApi, imgVarientDeleteApi, productVarientImgApi } from "@/apis/productApi";
...
interface ImageUploadPopupProps {
    ...
    productdata: any
    isOPenUploadImg: any
    isSelectedProductvarientId: any
}

const ImageVarientUploadPopup: React.FC<ImageUploadPopupProps> = ({ setOPenUploadImg, isSelectedProductImgId, productdata, isOPenUploadImg, isSelectedProductvarientId }) => {
    ...
    const [isSelectedProductImgIds, setSelectedProductImgId] = useState("")
    ...
    const handleCreateOrUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await productVarientImgApi(isSelectedProductImgId, sequence_number, image, is_active, created_by, token);
            ...
            >
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4 h-auto">
                    <Transition.Child
                        ...
                        <Dialog.Panel className="bg-white lg:h-auto h-full w-full max-w-4xl rounded-lg shadow-lg relative overflow-y-auto hidescroll">
                            <div className="flex justify-between items-center mb-4">
                                <div className="w-full">
                                    ...
                                    <form onSubmit={handleCreateOrUpdate} className="mb-4">
                                        <div className="flex justify-center items-center lg:flex-row flex-col w-full lg:p-6 p-3">
                                            <div className="flex p-4">
                                                {newImg.image ? (
                                                    <img
                                                        src={isImgPreview}
                                                        alt="Uploaded"
                                                        className="h-56 w-60 object-contain rounded-lg border border-gray-300"
                                                    />
                                                ) : (
                                                    <div className="h-56 w-60 flex items-center justify-center bg-gray-100 text-gray-400 rounded-lg border-[1px]">
                                                        No Image Uploaded
                                                    </div>
                                                )}
                                            </div>
                                ...
                                {isOpenDeletePopup && (<ProductImageDeletecomponent
                                    ...
                                    handleDeleteConform={() => handleDeleteConfirm(isSelectedProductImgIds)}
...
export default ImageVarientUploadPopup;
