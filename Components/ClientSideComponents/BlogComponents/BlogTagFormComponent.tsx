...
const BlogTagFormComponent = () => {
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        const response = await blogtagUpdatedApi(
          selectedRoleId,
          tagName,
          is_active,
          createdBy,
        ...
        const response = await createBlogTagApi(tagName,is_active, createdBy, token);
  ...
  const fetchTag = async () => {
    try {
      const response = await blogtagAllDataApi({
        page: currentPage,
        page_size: pageSize,
        token: token,
  ...
  const handleEdit = (data: { id: string; name: string,is_active:boolean }) => {
    setOpenForm(true);
    ...
    <div className='w-full flex flex-col lg:items-center' ref={topRef}>
      ...
      <div className='bg-white w-full shadow-md mt-6  flex justify-center items-center rounded-md'>
        <div className='w-full py-4'>
          ...
          {categry.length === 0 && (
            <div className='text-center p-4'>No tag available</div>
          )}
        </div>
      </div>
      {isOpenInfoPopup && (
...
export default BlogTagFormComponent;
