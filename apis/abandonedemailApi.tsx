...
export const abandonedAllDataApi = async (apiParams: {ordering: string, isActive?: boolean}, token: string) => {
  ...
  const endpoint = `/user/abandoned-cart-setting/?${queryParams}`;
  const response = await apiCoreGet(endpoint, "GET", token);
  return response;
};

export const abandonedDeleteApi = async (id: any, token: string) => {
  const response = await apiCoreDelete(`/user/abandoned-cart-setting/${id}/`, token);
  return response;
};
export const abandonedUpdatedApi = async (id: string, sendTime: string, discount: string, expireTime: string,isActive: boolean,token: string) => {
  ...
  const response = await apiCoreUpdate(`/user/abandoned-cart-setting/${id}/`, "", requestBody, "PATCH", token);
  return response;
};

export const abandonedCreateApi = async (sendTime: string, discount: string, expireTime: string,isActive: boolean,token: string) => {
  const response = await apiCores(`/user/abandoned-cart-setting/`, { hours_after_email_is_sent:sendTime,discount_to_be_given_in_percent:discount,hours_after_email_cart_is_emptied:expireTime, is_active: isActive }, "POST", token);
  return response;
};
